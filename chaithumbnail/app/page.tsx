...
const LoadingState = () => {
    ...
    return (
        <div className="flex flex-col items-center justify-center p-8 text-center animate-fadeIn">
            ...
            <div className="loader-animation"><div></div><div></div><div></div><div></div><div></div><div></div><div></div><div></div></div>
            <p className="mt-6 text-xl text-gray-300 font-semibold animate-pulse">{fact}</p>
            <p className="mt-2 text-md text-gray-400">Your thumbnail is being generated. Please wait...</p>
        </div>
    );
};
...
const App = () => {
  ...
  const [pillMoods, setPillMoods] = useState([]);
  ...
  const pillCategoryOptions = ['Cooking', 'Gaming', 'Tech', 'DIY', 'Travel', 'Vlog', 'Tutorial', 'Review'];
  const pillMoodOptions = ['Excited', 'Serious', 'Thumbs Up', 'Confused', 'Happy', 'Surprised', 'Shouting', 'Thinking', 'Winking'];

  const fileInputRef = useRef(null);
  const logoInputRef = useRef(null);
  const inspirationInputRef = useRef(null);

  const handlePillClick = (pill, setter) => {
    setter(prev => {
      if (prev.includes(pill)) {
        return prev.filter(p => p !== pill);
      } else {
        return [...prev, pill];
      }
    });
  ...
  const handleAddCustomPill = (e, customValue, setter, customSetter, currentPills) => {
    e.preventDefault();
    if (customValue.trim() !== '' && !currentPills.includes(customValue.trim())) {
      setter(prev => [...prev, customValue.trim()]);
      customSetter('');
    }
  };

  const handleInspirationSelection = async (inspiration) => {
    try {
  ...
  const generateThumbnail = async () => {
    ...
    if (API_KEY === "") {
      setError("API Key is missing. Please add your key to the code to proceed.");
      setLoading(false);
      return;
    }
    ...
    try {
      ...
      let prompt = `Create a high-quality YouTube thumbnail in a 16:9 aspect ratio. Use the uploaded image as the base.`;
      ...
      if (logo) {
        ...
        prompt += ` The third image is a brand logo. It must be placed in either the top-left or top-right corner of the thumbnail.`;
      }

      if (inspirationImage) {
        const base64Inspiration = inspirationImage.split(',')[1];
        const inspirationMimeType = inspirationImage.split(':')[1].split(';')[0];
        
        parts.push({
          inlineData: {
            mimeType: inspirationMimeType,
            data: base64Inspiration,
          },
        });
        prompt += ` Use the second image for style and composition inspiration.`;
      }

      prompt += ` Make the image eye-catching, with bold, clear text and vibrant colors. The final image should be a single, polished thumbnail.`;
      ...
      const payload = {
        contents: [{ parts }],
        generationConfig: {
            // Correct way to request an image from Gemini
      ...
      <div className="w-full max-w-4xl bg-gray-900 rounded-3xl shadow-xl p-8">
        ...
        <div className="grid md:grid-cols-2 gap-8 mb-8">
          ...
          <div className="flex flex-col items-center p-6 bg-gray-800 rounded-2xl border border-gray-700">
            <h2 className="text-xl font-semibold mb-4 text-blue-300">Your Base Photo<span className="text-red-500">*</span></h2>
            <div
              id="base-photo-dropzone"
              className={`w-full h-48 bg-gray-700 rounded-xl flex items-center justify-center cursor-pointer border-2 border-dashed transition-colors ${isDraggingBase ? 'border-blue-500' : 'border-gray-600'}`}
            ...
            >
              {image ? (
                <img src={image} alt="Base" className="rounded-lg max-h-full max-w-full object-contain" />
              ) : (
          ...
          <div className="flex flex-col items-center p-6 bg-gray-800 rounded-2xl border border-gray-700">
            <h2 className="text-xl font-semibold mb-4 text-purple-300">Your Inspiration<span className="text-red-500">*</span></h2>
            <div
              id="inspiration-photo-dropzone"
              className={`w-full h-48 bg-gray-700 rounded-xl flex items-center justify-center cursor-pointer border-2 border-dashed transition-colors ${isDraggingInspiration ? 'border-purple-500' : 'border-gray-600'}`}
              onClick={() => inspirationInputRef.current.click()}
              onDragOver={(e) => handleDragOver(e, setIsDraggingInspiration)}
              onDragLeave={(e) => handleDragLeave(e, setIsDraggingInspiration)}
              onDrop={(e) => handleDrop(e, setInspirationImage)}
            >
              {inspirationImage ? (
                <img src={inspirationImage} alt="Inspiration" className="rounded-lg max-h-full max-w-full object-contain" />
              ) : (
            ...
            <input
              ...
              className="hidden"
              onChange={(e) => {
                handleImageUpload(e.target.files[0], setInspirationImage);
              }}
              accept="image/*"
            />
          </div>
        ...
        <div className="mb-8 p-6 bg-gray-800 rounded-2xl border border-gray-700">
          ...
          <div className="flex flex-wrap gap-4 justify-center">
            {inspirationThumbnails.map(thumb => (
              <div
                key={thumb.title}
                className={`cursor-pointer rounded-xl overflow-hidden shadow-lg transition-transform transform hover:scale-105 ${inspirationImage === thumb.url ? 'ring-4 ring-blue-500' : ''}`}
                onClick={() => handleInspirationSelection(thumb)}
              >
                <img src={thumb.url} alt={thumb.title} className="w-48 h-27 object-cover" />
        ...
        <div className="mb-8 p-6 bg-gray-800 rounded-2xl border border-gray-700">
          ...
          <label htmlFor="thumbnailText" className="block text-lg font-semibold mb-2 text-gray-300">
          ...
          <label htmlFor="description" className="block text-lg font-semibold mt-6 mb-2 text-gray-300">
          ...
          <form onSubmit={(e) => handleAddCustomPill(e, customCategory, setPillCategories, setCustomCategory, allCategories)} className="flex gap-2">
            <input
              type="text"
            ...
            />
            <button type="submit" className="px-4 py-2 bg-blue-500 text-white rounded-full hover:bg-blue-600 transition-colors">Add</button>
          </form>
          ...
          <form onSubmit={(e) => handleAddCustomPill(e, customMood, setPillMoods, setCustomMood, allMoods)} className="flex gap-2">
            <input
              type="text"
            ...
            />
            <button type="submit" className="px-4 py-2 bg-purple-500 text-white rounded-full hover:bg-purple-600 transition-colors">Add</button>
          </form>
        </div>
        
        {/* Branding Section */}
        <div className="mb-8 p-6 bg-gray-800 rounded-2xl border border-gray-700">
            <h2 className="text-xl font-semibold mb-4 text-yellow-300">Add Your Branding (Optional)</h2>
            <div
                id="logo-dropzone"
                className={`flex flex-col items-center p-4 bg-gray-700 rounded-xl cursor-pointer transition-colors border-2 border-dashed ${isDraggingLogo ? 'border-yellow-500' : 'border-gray-600'}`}
            ...
            >
                ...
                <div className="w-32 h-32 bg-gray-600 rounded-full flex items-center justify-center border-2 border-dashed border-gray-500">
                    {logo ? (
                        <img src={logo} alt="Logo" className="rounded-full w-full h-full object-contain" />
                    ) : (
        ...
        <div className="mt-8">
          ...
          {generatedImage && (
            <div className="bg-gray-800 p-4 rounded-2xl flex flex-col items-center">
              <h2 className="text-xl font-semibold mb-4 text-green-400">Your Generated Thumbnail</h2>
              <img src={generatedImage} alt="Generated Thumbnail" className="rounded-xl w-full max-w-lg shadow-lg border-2 border-gray-700" />
              <button 
