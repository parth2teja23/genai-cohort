...
const detectFormFields = tool({
  ...
  description: 'Detect common signup fields (firstName, lastName, fullName, email, password, confirmPassword, submit) and return CSS selectors as JSON.',
  ...
  async execute() {
    const data = await page.evaluate(() => {
      ...
      for (const label of document.querySelectorAll('label')) {
        ...
        const s = selFor(el);
        if (!s) continue;
        if (/first.*name/.test(lab)) result.firstName ??= s;
        if (/last.*name|surname|family.*name/.test(lab)) result.lastName ??= s;
        if (/full.*name|name/.test(lab)) result.fullName ??= s;
        if (/email/.test(lab)) result.email ??= s;
        if (/password/.test(lab) && !/confirm|re[-\s]?type|re[-\s]?enter|repeat|again/.test(lab)) {
          result.password ??= s;
        }
        if ((/confirm|re[-\s]?type|re[-\s]?enter|repeat|again/.test(lab)) && /password/.test(lab)) {
          result.confirmPassword ??= s;
        }
      }

      // Inputs / buttons sweep
      for (const el of document.querySelectorAll('input,button')) {
        ...
        if (el.tagName === 'INPUT') {
          ...
          const looksPassword = el.type === 'password' || /(password|pass)/.test(a);
          const looksConfirm = /(confirm|re[-\s]?type|re[-\s]?enter|repeat|again)/.test(a);
          if (!result.password && looksPassword && !looksConfirm) result.password = s;
          if (!result.confirmPassword && looksPassword && looksConfirm) result.confirmPassword = s;
        }
...
3) Call 'detect_form_fields' to retrieve JSON with selectors: firstName, lastName, fullName, email, password, confirmPassword, submit.
4) Using those selectors:
   - If 'fullName' exists, fill it with the full name from the task.
   - Else fill 'firstName' and 'lastName' separately.
   - Fill 'email' and 'password'.
   - If 'confirmPassword' exists, fill it with the SAME password string as in the task.
   - Then click 'submit' if present.
5) After the fill/submit sequence, call 'handoff_to_vision' once to verify state.
6) Stop when the form appears submitted or a confirmation appears.

Rules:
- Use 'fill_selector' for inputs; do not type character by character.
...
- Keep steps minimal and deterministic.
  ...
  tools: [
    ...
    handoffToVision,
    detectFormFields,
    handoffToKeyboard,  // uses fill_selector
    handoffToMouse,     // for submit click
  ],
});
