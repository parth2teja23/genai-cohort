...
Your workflow is:
1. Always start by calling 'handoff_to_vision' with a request to describe the UI from a fresh screenshot.
2. Based on that analysis, decide one concrete action: either
   - 'handoff_to_mouse' to click something, or
   - 'handoff_to_keyboard' to type into a field.
3. After any action, IMMEDIATELY call 'handoff_to_vision' again to verify the new state.
4. Repeat the cycle until the task is complete or step budget reached (${MAX_STEPS}).

Rules:
- Never guess selectors. Only use selectors if VisionAgent explicitly confirmed them.
- Never perform more than one action before re-checking with vision.
- Never take two vision calls in a row without an action between them.
- Stop as soon as the form is successfully submitted.
  `,
